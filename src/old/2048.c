...
static void set_line(index, line) int index; int *line; {
	int i;
	for (i = 0; i < LINE_SIZE; ++i)
		e_board[index * LINE_SIZE + i] = line[i];
}

static int compare(line_first, line_second) int *line_first; int *line_second; {
	int i;
	for (i = 0; i < LINE_SIZE; ++i)
...
static int *get_line(index, reg) int index; int *reg; {
	int i;
	for (i = 0; i < LINE_SIZE; ++i)
...
static int *move_line(line) int *line; {
	int size = 0, i;
	for (i = 0; i < LINE_SIZE; ++i)
		if (line[i])
			f_reg[size++] = line[i];
...
static int *merge_line(line) int *line; {
	int size = 0, i;
	reset_b_reg();
...
static void add_tile(n) int n; {
	int i;
	for (i = 0; i < n; ++i) {
		int size = update_space();
		if (size)
...
extern void e_key(keycode) int keycode; {
	if (keycode == K_ESCAPE)
		reset();
	if (!can_move())
		e_lose = 1;
